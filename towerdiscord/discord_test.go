...
func TestDiscord(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantCount: 1,
			context:   []any{tower.F{"foo": "bar"}},
			error:     nil,
			message:   "foo",
			extraOpts: []DiscordOption{
				WithName("test-discord"),
				WithCache(cache.NewLocalCache()),
				WithSemaphore(make(chan struct{}, 3)),
				WithTrace(tower.TraceCaptureFunc(func(ctx context.Context) tower.Trace {
					return tower.Trace{{Key: "foo", Value: "bar"}}
				})),
				WithBucket(nil),
